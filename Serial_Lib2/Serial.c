...
/* 1次バッファ */
#define SERIAL_TMP_BUFSIZE	1024
...
DWORD WINAPI serial_thread(LPVOID param)
{
	...
	while ( obj->thread_active ) {
		...
		if ( recv_len )
		{
			...
			recv_buf[recv_len]=0;
			recv_len -= fifo_write_64(obj->q_recv, recv_buf, recv_len);
		...
		if( send_size > 0 )
		{
			...
			if ( send_size != send_len ){
				obj->msg = "WriteFile spilled some of q_send.";
			}
