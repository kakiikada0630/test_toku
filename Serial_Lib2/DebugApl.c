...
void Analize_PWM( unsigned char* buf, int size, struct Parameter *param )
{
	...
	param->PWM8   = *(buf16+8);
	param->PWM9   = *(buf16+9);
	param->PWM10  = *(buf16+10);
	param->PWM11  = *(buf16+11);
	param->PWM12  = *(buf16+12);
	param->PWM13  = *(buf16+13);
}


void Analize_SPI( unsigned char* buf, int size, struct Parameter *param )
{
	#define MAX_CMD_SIZE  SPITX_SIZE/2
	#define BST_CMD_SIZE  6

	unsigned short *buf16         = (unsigned short *)(buf+BLOCK_SPITX);
	...
	//--------------------------------------------------
	buf16 = (unsigned short *)(buf+BLOCK_SPITX+BST_CMD_SIZE*2); // Buckコマンドの先頭位置へ移動
	...
	//--------------------------------------------------
	buf16 = (unsigned short *)(buf+BLOCK_SPITX+BST_CMD_SIZE*2+buck_cmd_size*2); // Buckコマンドの先頭位置へ移動
	Ch1adj = 0;
	Ch2adj = 0;
...
void Analize_DAC( unsigned char* buf, int size, struct Parameter *param )
{
	...
	param->DAC0  = *(dac_pnt  );
	param->DAC1 = *(dac_pnt+1);
	param->DAC2 = *(dac_pnt+2);
	param->DAC3 = *(dac_pnt+3);
	param->DAC4 = *(dac_pnt+4);
	param->DAC5 = *(dac_pnt+5);
	param->DAC6 = *(dac_pnt+6);
	param->DAC7 = *(dac_pnt+7);
}

void Analize_ADC( unsigned char* buf, int size, struct Parameter *param )
{
	short *dac_pnt = (short *)(buf+BLOCK_ADC);
	
	param->ADC0 = *(dac_pnt  );
	param->ADC1 = *(dac_pnt+1);
	param->ADC2 = *(dac_pnt+2);
	param->ADC3 = *(dac_pnt+3);
	param->ADC4 = *(dac_pnt+4);
	param->ADC5 = *(dac_pnt+5);
	param->ADC6 = *(dac_pnt+6);
	param->ADC7 = *(dac_pnt+7);
}
...
void WriteLog( unsigned char* buf, int size, struct Parameter *param )
{
	...
	// PWM出力
	buf32 = (unsigned int*)(buf+BLOCK_PWM);
	loop = ( PWM_SIZE )/ 4;
	for( int i=0 ; i<loop  ; i++ )
	{
		sprintf( out+i*6, "%04x, ", *(buf32+i) );
	}
	out += loop*6; // PWM領域分のアドレスを進める

	// SPITX
	loop = ( SPITX_SIZE );
	buf8 = buf + BLOCK_SPITX;
	...
	// ADC
	loop = ( ADC_SIZE )/2;
	buf16 = (unsigned short*)(buf + BLOCK_ADC);
	for( int i=0 ; i<loop  ; i++ )
	{
		sprintf( out+i*6, "%04x, ", *(buf16+i) );
	}
...
void FileOpenInt()
{
	...
	sprintf( out, "[Tick(ms)],PWM0,PWM1,PWM2,PWM3,PWM4,PWM5,PWM6,PWM7,PWM8,PWM9,PWM10,PWM11,PWM12,PWM13,SPI1,SPI2,UART1,LIN,CAN,DAC0,DAC1,DAC2,DAC3,DAC4,DAC5,DAC6,DAC7,ADC0,ADC1,ADC2,ADC3,ADC4,ADC5,ADC6,ADC7,SW\n");
...
DLLAPI void OpenSerial(char* com_dbg, char* com_plusb)
{
	...
	sys_t.obj = serial_create(com_dbg,4000000,com_plusb,9600);
	if ( sys_t.obj == NULL ) {
