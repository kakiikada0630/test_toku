...
{
	int lev= gpio_get_level(GPIO_UDIM21);
...
void init_gpio_UDIM21()
{
	gpio_config_t config = {
		.pin_bit_mask = (((uint64_t) 1) << GPIO_UDIM21),
    ...
    gpio_isr_handler_add(GPIO_UDIM21, gpio_isr_handler, (void*) GPIO_UDIM21);
}

int32_t get_percent_UDIM21()
{
	int64_t cur_time;
	int32_t buf=0;
	...
	gpio_intr_disable(GPIO_UDIM21);
	buf = (int32_t)(cur_time - latest_time);
	gpio_intr_enable(GPIO_UDIM21);

	//最新の割り込み時間から7ms以上経過している場合、Dutyが
	//0%か100.00%に張り付いていると判断する。
	if( buf > 7000 )
	{
		per = ( gpio_get_level(GPIO_UDIM21)==0 )? 0 : 10000;
	}
