...
{
	int lev= gpio_get_level(GPIO_B2);
...
void init_gpio_B2()
{
	gpio_config_t config = {
		.pin_bit_mask = (((uint64_t) 1) << GPIO_B2),
    ...
    gpio_isr_handler_add(GPIO_B2, gpio_isr_handler, (void*) GPIO_B2);
}

int32_t get_percent_B2()
{
	int64_t cur_time;
	int32_t buf=0;
	...
	if( buf > 7000 )
	{
		per = ( gpio_get_level(GPIO_B2)==0 )? 0 : 10000;
	}
