...
{
	int lev= gpio_get_level(GPIO_B3);
...
void init_gpio_B3()
{
	gpio_config_t config = {
		.pin_bit_mask = (((uint64_t) 1) << GPIO_B3),
    ...
    gpio_isr_handler_add(GPIO_B3, gpio_isr_handler, (void*) GPIO_B3);
}

int32_t get_percent_B3()
{
	int64_t cur_time;
	int32_t buf=0;
	...
	gpio_intr_disable(GPIO_B3);
	buf = (int32_t)(cur_time - latest_time);
	gpio_intr_enable(GPIO_B3);

	//最新の割り込み時間から7ms以上経過している場合、Dutyが
	//0%か100.00%に張り付いていると判断する。
	if( buf > 7000 )
	{
		per = ( gpio_get_level(GPIO_B3)==0 )? 0 : 10000;
	}
